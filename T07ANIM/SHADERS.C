 ...
 */
static VOID AS3_SaveLog( CHAR *Text )
{
  FILE *F;
 ...
 */
static CHAR * AS3_TextLoad( CHAR *FileName )
{
  FILE *F;
  CHAR *mem = NULL;
  ...
  return mem;
} /* End of 'AS3_ShaderLoad' function */
...
UINT AS3_ShaderLoad( CHAR *FileNamePrefix )
{
  ...
  for (i = 0; i < NumOfShaders; i++)
  {
    ...
    {
      isok = FALSE;
      AS3_SaveLog("Error create shader");
      break;
    }
    /* загружаем файл */
    if ((txt = AS3_TextLoad(Buf)) == NULL)
    {
      isok = FALSE;
      AS3_SaveLog("Error load file");
      AS3_SaveLog(Buf);
      break;
    }
    /* закрепляем текст */
...
} /* End of 'AS3_ShaderFree' function */
